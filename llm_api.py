...
4. ENGLISH: ENGLISH: קיימות אותיות באנגלית ב-TRANSLATED. התעלם (אל תפסול) אם מדובר בשם מותג מובהק, ראשי תיבות, איות של מילה (כמו S-I-F-U) או אותיות בודדות המשמשות לתיאור צורה/סמל (כמו האות I, צורת V או הסימון X). אם האות האנגלית נמצאת שם מסיבה לוגית ומוצדקת זו - החזר true.
5. SEMANTIC FIDELITY & ALLOWED DRIFT (תוכן וזליגה): **פסול (false) רק אם** יש שינוי משמעות **מהותי**, סתירה לוגית למקור, המצאה (Hallucination), או השמטת מונח מפתח שלא מופיע בשום אינדקס סמוך ב-OVERLAP ובגוף הבאץ' הנבדק. **זליגה מותרת (Shifting):** העברת ניסוח בין אינדקסים **סמוכים** לטובת עברית טבעית — **מותרת** אם **מסת הקול** נשמרת ביחד עם ה-OVERLAP. **לפני פסילה על «השמטה»:** וודא שהחומר «החסר» לא מופיע בתרגום האינדקס **הקודם או הבא** (כולל שורות OVERLAP). **כשל קריטי לדוגמה:** שינוי פעולה/כוח (למשל «מורה» ↔ «אוסר»), או אובדן מונח מפתח מהמקור בכל הבאץ'+הקשר. **ספק לטובת המתרגם:** אם חוקים **1–4** מתקיימים והמשמעות הכללית נשמרת עם ה-OVERLAP — **אשר (true)**.

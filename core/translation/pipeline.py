...
def run_pipeline(self, config):
    try:
        ...
        try:
            cfg, session_state = initialize_pipeline_session(config, self.log_queue, self.ui_queue, self.shared_state)
        ...
        current_index = session_state.progress.current_index
        processed = session_state.progress.processed
        session_processed = session_state.progress.session_processed
        success_streak = session_state.batching.success_streak
        
        total_main_cost = session_state.total_main_cost
        total_judge_cost = session_state.total_judge_cost
        context_state = session_state.context_state
        translated_target_by_index = session_state.translated_target_by_index
        ...
        with open(output_file, file_mode, encoding='utf-8') as f_out:
            while current_index < total_blocks and not self.should_stop:
                ...
                while not batch_state.batch_success and not self.should_stop:
                    batch_diagnostics_logged = False
                    native_audit_reason = ""
                    batch_state.attempted_batch_sizes.append(batch_state.current_batch_size)
                    start_idx = current_index
                    end_idx = min(current_index + batch_state.current_batch_size, total_blocks)
                    ...
                    try:
                        ...
                        is_retry = (len(batch_state.attempted_batch_sizes) > 1)
                        ...
                        received_dict, res_json = process_llm_response(
                            ...
                            session_log_file, indices, batch_state.current_batch_size, 
                        ...
                        batch_passed, last_judge_error, last_judged_indices, j_cost_delta, previous_overlong_indices = run_audit_pipeline(
                            ...
                            current_batch_size=batch_state.current_batch_size,
                        ...
                        session_state.progress = replace(
                            session_state.progress,
                            processed=processed,
                            session_processed=session_processed
                        )
                        
                        evaluate_batch_success(batch_state, batch_size, self.log_queue, session_log_file, stats)
                        effective_batch_size = batch_state.effective_batch_size
                        success_streak = batch_state.success_streak
                        ...
                        evaluate_batch_failure(batch_state, self.log_queue, session_log_file, stats)
                        if batch_state.min_batch_failures >= 3:
                            result = execute_manual_intervention_or_bypass(
                                self, batch_state, indices, expected_count, original_metadata, eng_by_index,
                            ...
                            if result.batch_success:
                                ...
                                session_start_time = updates.get("session_start_time", session_start_time)
                                received_dict = updates.get("received_dict", received_dict)
                                
                                if updates.get("is_bypass", False):
                                    bypass_count = updates["bypass_count"]
                                    if bypass_log_file is None:
                                        bypass_log_file = self._create_bypass_log(session_log_file)
                                    self._write_bypass_entry(
                                        bypass_log_file, updates["eng_src_for_intervention"], 
                                        updates["bypass_dict"], updates["reason_for_human"]
                                    )
                                    
                                self._finalize_batch_success(
                                    original_metadata, received_dict, f_out,
                                    translated_target_by_index, updates["res_json"], context_state,
                                    stats, indices, expected_count, pipeline_load, pipeline_start_time, 
                                    target_is_rtl=profile.target_is_rtl
                                )
                                batch_state.min_batch_failures = 0
                                batch_state.failures_at_current_size = 0
                                batch_state.batch_success = True
                                break

                if batch_state.batch_success:
